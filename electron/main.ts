...
// ============================================================
// v5.12: ESC/POS DIRECT PRINTING — The real thermal printer fix
// ============================================================
// Generates raw ESC/POS commands and sends them directly to the
// thermal printer via USB. This COMPLETELY BYPASSES:
//   - The browser print engine
//   - The Windows print spooler
//   - The print dialog
//   - A4 paper size defaults
//
// The output is perfectly formatted for 58mm or 80mm thermal paper.
// This is how ALL POS systems print receipts.
...
ipcMain.handle('print:escpos', async (_, data: {
  ...
  balanceDue: number
  paperWidth: string  // '58mm' or '80mm'
}) => {
  try {
    ...
    // Select the first USB printer
    const device = new escposUSB.USB()
    const printer = new escpos.Printer(device, { width: data.paperWidth === '58mm' ? 32 : 48 })
    ...
    await new Promise<void>((resolve, reject) => {
      device.open((err: any) => {
        if (err) {
          reject(new Error('Could not open USB printer: ' + err.message))
          return
        }
        ...
        if (data.sellerAddr) printer.text(data.sellerAddr)
        if (data.sellerPhone) printer.text('Ph: ' + data.sellerPhone)
        if (data.sellerGst) printer.text('GSTIN: ' + data.sellerGst)
        ...
        printer
          ...
          .feed(1)
          .align('LT')
          .text('Invoice: ' + data.invNo)
          .text('Date: ' + data.invDate)
          ...
          .text(data.buyerName)
          .style('NORMAL')
          if (data.buyerAddr) printer.text(data.buyerAddr)
          .feed(1)

        // Items header
        printer
          .style('B')
          .text('Item'.padEnd(20) + 'Qty'.padStart(6) + 'Total'.padStart(10))
          .style('NORMAL')
          .text('-'.repeat(36))
        ...
        for (const item of data.items) {
          ...
          const qty = `${item.qty} ${item.unit || ''}`.substring(0, 6)
          const total = 'Rs ' + item.total.toFixed(2)
          printer.text(itemName.padEnd(20) + qty.padStart(6) + total.padStart(10))
        }

        printer
          .text('-'.repeat(36))
          .feed(1)
          .text('Subtotal:'.padEnd(26) + ('Rs ' + data.subtotal.toFixed(2)).padStart(10))
          .text('Tax:'.padEnd(26) + ('Rs ' + data.taxAmount.toFixed(2)).padStart(10))
          .style('B')
          .size(1, 1)
          .text('GRAND TOTAL:'.padEnd(26) + ('Rs ' + data.totalAmount.toFixed(2)).padStart(10))
          .style('NORMAL')
          .size(1, 1)
          .text('Received:'.padEnd(26) + ('Rs ' + data.amountReceived.toFixed(2)).padStart(10))
          .text('Balance Due:'.padEnd(26) + ('Rs ' + data.balanceDue.toFixed(2)).padStart(10))
          .feed(2)
          .align('CT')
          .text('Authorised Signatory')
          .text('For ' + data.sellerName)
          .feed(1)
          .text('Computer-generated by BizBook Pro')
          .text('Tahigo International')
          .feed(2)
          .cut()
          .close(() => {
            console.log('[escpos] Print completed')
            resolve()
          })
      })
    })
