...
export function SaleRegister() {
  ...
  // These are the SAME financial formulas — NO calculation logic changed.
  // The memo dependencies are [items, form.discountPercent] so the values
  // only recompute when items or the discount percent actually change.
  //
  ...
  const handlePrintInvoice = (sale: Sale) => {
    ...
    // v6.25.19: QR amount depends on invoice status — EXACT per user spec:
    //   Quotation           → UPI amount if > 0, else Grand Total
    ...
    const qrPayAmount = isQuotationPrint
      ? (sale.upiAmount > 0 ? sale.upiAmount : sale.totalAmount)  // Quotation → UPI amount, fallback to total
      : printBalanceDue                                            // Invoice → Balance Due
    const showQr = upiId && qrPayAmount > 0 && (tenant as any)?.showQrCode !== false
